...
<font style="color: #006ec7 ">Internal vs. external trace models.</font> &emsp; The internal trace model of the ATL transformation engine enables the convenient setting of target element features and stopping rule execution. Rules are only executed if the matched input elements are not covered by an existing trace link.
...
> 
>	 * The Epsilon Transformation Language (ETL) supports out-place model transformations and allows modifying input model elements during transformations.